...
def build_features(df: pd.DataFrame, elevation_m: float = 201.0) -> pd.DataFrame:
    df = df.copy()
    L = 0.0065
    R = 287.058
    g0 = 9.80665
    ...
    tk = df["T_db"] + 273.15
    df["theta"] = tk * (p0 / df["SLP"]) ** (R * L / g0)

    a = 6.1121
    b = 17.368
    c = 238.88
    es = a * np.exp((b * df["T_dew"]) / (c + df["T_dew"]))
    e = a * np.exp((b * df["T_db"]) / (c + df["T_db"]))
    df["VPD"] = e - es

    wd_rad = np.radians(df["WD"])
    df["u"] = df["WS"] * np.sin(wd_rad)
    df["v"] = df["WS"] * np.cos(wd_rad)
...
def preprocess(
    ...
    X_train, y_train, t_idx = sliding_windows(train_df, W, horizons, target_col)
    X_val, y_val, t_idx2 = sliding_windows(val_df, W, horizons, target_col)
    X_test, y_test, t_idx3 = sliding_windows(test_df, W, horizons, target_col)
