...
├── README.md                       # This file
├── docs/
│   ├── paper/main.tex              # Write-up (twocolumn, <=5 body pages, see docs/paper/main.pdf)
│   ├── evaluation_protocol.md      # Fairness protocol every baseline (and the QRC) is held to
│   └── ipc_matching.md             # IPC-matched reservoir tuning methodology
├── scripts/
│   ├── reproduce.sh                # Entry point: --quick (<30min classical) / --full (supplementary robustness check)
│   ├── generate_baselines.py       # Classical fairness-protocol baselines (val/test)
│   ├── phase1_v5_canonical_drive.py, phase5_v4_canonical_drive.py  # v5/v4 QRC canonical-split GPU drives
│   ├── phase5_final_benchmark.py   # Combines classical + QRC results into results/full_matched_benchmark_*.json
│   ├── phase_nwp_baseline.py       # Real NWP (GFS via Open-Meteo) comparison baseline
│   └── make_paper_tables.py        # Regenerates every LaTeX table in the paper from results/*.json
├── configs/v5_reference.yaml       # Frozen v5 reservoir config
├── results/                        # results/*.json — the only source of truth for reported numbers
├── figures/                        # Vector (PDF) figures used in the paper
...
└── QRCx/                           # Installable package root
    ...
    └── QRCx/                        # Package source
        ...
        ├── reservoir/sequential.py    # SequentialDissipativeQRC — v5 recurrent dissipative reservoir (exact propagator default, Trotter kept for hardware-matched runs; multiplexing=V)
        ├── reservoir/sequential_backends.py # qiskit_aer / pennylane_mixed drivers, benchmarked against the numpy backend above
        ├── reservoir/tfim_cudaq.py    # CUDA-Q kernel for the windowed (v4) architecture (not yet dissipation-aware; not used in headline numbers, see paper Sec. 3.1)
        ...
        ├── metrics/                   # forecast.py (RMSE/MAE/skill/VPT), fsdh.py, significance.py (DM test, bootstrap CIs), reservoir.py / reservoir_sequential.py (MC/IPC)
        └── experiment/                # runner.py, benchmark.py, ablation.py, figures.py, figstyle.py
```
